...
export class AddCustomerComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.newCustomerFormGroup = this.fb.group({
      name: this.fb.control(null, [Validators.required, Validators.minLength(4), Validators.required]),
      email: this.fb.control(null, [Validators.email, Validators.required],),

    })
  }

  addCustomer(newCustomerFormGroup: FormGroup) {
    if (newCustomerFormGroup.invalid) {
      return;
    }
